...
PBS.KIDS.storybook.book = function (GLOBAL, PBS, storybookContainerElement, config) {
	...
	var sb = PBS.KIDS.storybook,
		...
		replayPageButtonSprite,
		homePageButtonSprite,
		pageTurnDuration,
		pageSlideDuration,
		navigating = false,
		cover,
		...
		replayPageButtonElement,
		homePageButtonElement,
		...
		pageDraggedRight = function (page) {
			...
			if (page === pages[leftPageIndex]) {
				...
				//     because it would do nothing on a real book. Top Comment.
			}
		},
		...
		start = function () {
			...
			// Draw replay page button and add event listener
			if (replayPageButtonSprite) {
				replayPageButtonSprite.update();
				replayPageButtonSprite.render();
				replayPageButtonSprite.addEventListener("PRESS", that.replayPage);
			}

			if (homePageButtonSprite) {
				homePageButtonSprite.update();
				homePageButtonSprite.render();
				homePageButtonSprite.addEventListener("PRESS", that.homePage);
			}
		...
		updateLoadMessage = function (e) {
			...
			if (audioPlayer) {
				if (audioLoadInitiated) {
					...
					loadingContainerElement.innerHTML = '<p id="loadingText">Loading ' + loadEvent.progress + " of " + (loadEvent.total + 1) + "<br />Click to Continue.</p>";
				}
			} else {
		...
		init = function () {
	      ...
	      if (bookConfig.previousPageButton) {
		      ...
		      prevPageButtonElement.id = "pbsPrevPageButton";
				...
				if(bookConfig.replayPageButton) {
					replayPageButtonElement = GLOBAL.document.createElement("div");
					replayPageButtonElement.id = "pbsReplayPageButton";
					replayPageButtonElement.className = "pbsPageButton";

					// create replay button sprite
					bookConfig.replayPageButton.parentElement = replayPageButtonElement;
					bookConfig.replayPageButton.resource = resourceLoader.addToQueue(bookConfig.replayPageButton.url);
					replayPageButtonSprite = sb.sprite(GLOBAL, PBS, bookConfig.replayPageButton);
					replayPageButtonSprite = sb.makeInteractionObject(GLOBAL, PBS, replayPageButtonSprite);
					navElement.appendChild(replayPageButtonElement);
				}

				if(bookConfig.homePageButton) {
					homePageButtonElement = GLOBAL.document.createElement("div");
					homePageButtonElement.id = "pbsHomePageButton";
					homePageButtonElement.className = "pbsHomeButton";

					// create home button sprite
					bookConfig.homePageButton.parentElement = homePageButtonElement;
					bookConfig.homePageButton.resource = resourceLoader.addToQueue(bookConfig.homePageButton.url);
					homePageButtonSprite = sb.sprite(GLOBAL, PBS, bookConfig.homePageButton);
					homePageButtonSprite = sb.makeInteractionObject(GLOBAL, PBS, homePageButtonSprite);
					navElement.appendChild(homePageButtonElement);
				}
				...
				//Play with Narration?
				playNarration = confirm("Play with Narration?");
				

				if (config.audio && config.audio && config.audio.name && config.audio.enabled && playNarration !== false) {
...
// TODO: Check if window.resize gets called on orientation change. If it is the following is unneccessary.
	...
	that.replayPage = function() {
		console.log("Replay Button Pressed!");
		navigateToPageIndex(curPageIndex, true);

	}

	that.homePage = function() {
		navigateToPageIndex(-1,true);
	}
