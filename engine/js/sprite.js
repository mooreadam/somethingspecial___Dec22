...
PBS.KIDS.storybook.sprite = function (GLOBAL, PBS, options) {
	...
	var that,
		...
		startDelay = options.startDelay || 1,
		hasStartBeenDelayed = false,
		loop = options.loop,
...
//console.log(totalNumFrames + " * " + curTick + "/" + totalTicks + " = " + currentFrame);
	...
	var startDelayTimeElapsed = 0;

	that.update = function () {

		// Display the current amount of start delay if object has one
		if(options.startDelay && !hasStartBeenDelayed) {
			//console.log("[StartDelay] Currently "+startDelayTimeElapsed+"/"+options.startDelay);
		}
		...
		if (numFrames && !paused) {
			...
			//If frame has a start delay
			if(startDelay != 0 && hasStartBeenDelayed == false) {

				//Delay Sprite
				startDelayTimeElapsed++;

				// If delay time is up, set flag
				if(startDelayTimeElapsed > startDelay) {
					hasStartBeenDelayed = true;

				}
			}


			if(hasStartBeenDelayed == true) {
